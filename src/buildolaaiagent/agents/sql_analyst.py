...
from buildolaaiagent.models.schema import AgentSchema,JudgeSchema
from buildolaaiagent.utils.database import DatabaseUtil
from langgraph.graph import StateGraph ,START ,END
import re
# llm_object = pick_llm("easy")
# print(llm_object.invoke("Who are you?"))
...
def curated_questions(state : AgentSchema)-> AgentSchema:
...
def generate_sql(state: AgentSchema) -> AgentSchema:
    ...
    cleaned_query = re.sub(r"^```(?:sql)?\s*", "", raw_query.strip(), flags=re.IGNORECASE)
...
def execute_sql(state: AgentSchema) -> AgentSchema:
    ...
    state.sql_query_exec_result = execution_result
...
def represent_final_answer(state: AgentSchema) -> AgentSchema:
    ...
    state.messages = state.messages + [AIMessage(content=llm_response)]  # Append the final answer to the messages list
...
# Nodes
sql_agent_graph.add_node(curated_questions,name="curated_questions")
...
# Edges
sql_agent_graph.add_edge(START, "curated_questions")
sql_agent_graph.add_edge("curated_questions", "prompt_query_context")
...
if __name__ == "__main__":
    ...
    # print(sql_analyst_response['sql_query_exec_result'])  # Print the result of executing the SQL query
