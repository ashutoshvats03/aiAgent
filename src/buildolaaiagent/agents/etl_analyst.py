...
from buildolaaiagent.models.schema import ETLAgentSchema
...
def tool_node(state:ETLAgentSchema):
    ...
    # Here the "messages" has all the data [prompt , args , tool_calls to make , args of the tools] because the argumnets we have written there
    # so we will extract the tool_calls list and run all the tools it needs
    ...
    # Till the time LLM is not staisfied it will use the messages context with ReAct agent
